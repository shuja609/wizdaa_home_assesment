...
export class RequestsService {
  ...
  async createRequest(dto: CreateRequestDto): Promise<TimeOffRequest> {
    ...
    if (days === 0) {
      throw new BadRequestException('Request does not include any business days');
    }

    // Local balance pre-check
    const balances = await this.balancesService.getBalances(dto.employeeId, dto.locationId);
    const balance = balances.find((b) => b.leaveType === dto.leaveType);

    if (!balance || balance.balance < days) {
      throw new BadRequestException(`Insufficient ${dto.leaveType} balance. Required: ${days}, Available: ${balance?.balance ?? 0}`);
    }
  ...
  async approveRequest(id: string, managerId: string): Promise<TimeOffRequest> {
    ...
    const balances = await this.balancesService.syncBalances(request.employeeId, request.locationId);
    const balance = balances.find((b) => b.leaveType === request.leaveType);

    // 2. Re-validate balance
    if (!balance || balance.balance < request.days) {
      throw new BadRequestException(`Insufficient ${request.leaveType} balance at HCM. Available: ${balance?.balance ?? 0}, Required: ${request.days}`);
    }
  ...
  async cancelRequest(id: string): Promise<TimeOffRequest> {
    ...
    if (request.status === RequestStatus.APPROVED) {
      ...
      const hoursSinceRequest = (Date.now() - new Date(request.requestedAt).getTime()) / (1000 * 60 * 60);
      
      if (hoursSinceRequest > graceHours) {
        throw new BadRequestException(`Cancellation window (${graceHours}h) has expired for this approved request`);
      }
      ...
      try {
        ...
        if (!hcmResult.success) {
          this.logger.error(`Failed to credit HCM on cancellation for request ${id}`);
          // Still cancel locally but log the failure
        }
      } catch (error) {
        this.logger.error(`HCM communication error during cancellation: ${error.message}`);
      }
    }
